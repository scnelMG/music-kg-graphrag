...
def test_scope_audit_ignores_generated_variant_directories(tmp_path: Path) -> None:
    # Given
    source = tmp_path / "source"
    for directory_name in (".next-e2e-3199", ".playwright-owner-readiness", ".tmp-task-run"):
        generated = source / "frontend" / directory_name
        generated.mkdir(parents=True)
        _ = (generated / "generated.js").write_text("GENERAL_CHATBOT = true\n", encoding="utf-8")
    backend = source / "backend"
    backend.mkdir()
    _ = (backend / "service.java").write_text("final class Service {}\n", encoding="utf-8")
    output = tmp_path / "scope.json"
