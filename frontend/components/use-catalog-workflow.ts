...
export function useCatalogWorkflow({ ownerAccess, recordState }: CatalogWorkflowOptions) {
  ...
  async function loadTracks(album: Album, edition: CatalogEdition, existing?: ExistingRecord): Promise<void> {
    ...
    setFavouriteTrack("");
    setSelectedTrack(undefined);
    const outcome = await requestBff(ky.get(`/api/music/albums/${encodeURIComponent(album.releaseGroupMbid)}/tracks`, {
    ...
    const savedTrack = existing?.youtubeRecordingMbid.length
  ...
  async function loadMoreEditions(): Promise<void> {
    if (selected === null || editionCursor === null || loadingMoreEditions) return;
  ...
  async function resolveExistingRecord(album: Album, generation: number): Promise<void> {
    const outcome = await requestBff(ky.get(
  ...
  async function search(nextQuery = query.trim()): Promise<void> {
    ...
    setAlbums([]);
    resetSelection();
    const outcome = await requestBff(ky.get("/api/music/albums", {
  ...
  return { albums, editionMessage, editionState, editions, favouriteTrack, hasMoreEditions, loadingMoreEditions,
    ...
    selectTrack, setOwned, setQuery, setSentiment, submitSearch, selectAlbum, selectEdition, loadMoreEditions,
