...
import { catalogIdentity, type CatalogEdition } from "../lib/music-catalog-contract";
...
type MusicCatalogSectionProps = {
  ...
  readonly onSearchExample: (example: string) => void;
  ...
  readonly recordedCatalogIdentities: ReadonlySet<string>;
...
export function MusicCatalogSection(props: MusicCatalogSectionProps): React.JSX.Element {
  let prioritizedCoverCount = 0;
  return <>
    <section className="search-section" id="candidate-search">
      {props.ownerAccess === "owner" && <p className="section-kicker">음반 찾기</p>}<h2 id="search-heading">듣고 싶은 앨범을 찾아보세요.</h2>
      <p className="instruction">앨범명이나 가수명으로 앨범과 수록곡을 찾습니다.</p>
      ...
      {props.searchState !== "idle" && <div className="result-region" aria-live="polite" aria-busy={props.searchState === "loading"}>
