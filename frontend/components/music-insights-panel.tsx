      ...
      {props.graphTaste === null
        ...
        : <>
          ...
          {featuredAlbum === null
            ...
            : <section className="today-recommendation"><AlbumArt album={featuredAlbum} priority size="hero" /><div className="today-recommendation-copy"><p className="entry-eyebrow">오늘의 추천</p><strong>{featuredAlbum.title}</strong><span>{featuredAlbum.artist}</span>{todayRecord !== undefined && <button className="today-recommendation-action" type="button" onClick={() => props.onOpenRecord(todayRecord)}>기록 보기</button>}{todayRecord?.youtubeVideoId.length === 11 && todayRecord.youtubeRecordingMbid.length > 0 && todayRecord.youtubeVideoTitle.length > 0 && todayRecord.youtubeChannelTitle.length > 0 && <YouTubePlayback video={{ channelTitle: todayRecord.youtubeChannelTitle, recordingMbid: todayRecord.youtubeRecordingMbid, thumbnailUrl: "", title: todayRecord.youtubeVideoTitle, videoId: todayRecord.youtubeVideoId }} />}</div>{todayRelisten !== null && <details className="recommendation-reason"><summary>추천 이유</summary><p>{relistenReason(todayRelisten, props.ownerAccess, props.graphTaste.relisten.length)}</p></details>}</section>}
