...
export function PersonalCoverRail({ onSelectRecord, records }: PersonalCoverRailProps): React.JSX.Element | null {
  ...
  return <section className="personal-cover-rail" data-testid="personal-cover-rail" aria-labelledby="recent-records-heading">
    ...
    <ul className={`cover-rail-scroller${records.length === 1 ? " is-single" : ""}`}>
      {records.slice(0, 7).map((record, index) => <li className="cover-rail-list-item" key={record.recordHandle}><button className={`cover-rail-item${records.length === 1 ? " is-single" : ""}`} type="button" onClick={() => onSelectRecord(record)} aria-label={`${record.albumTitle} 기록 수정`}>
