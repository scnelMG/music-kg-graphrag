...
export function PublicDiscoveryDeck({ albums, label, onOpenAlbum }: PublicDiscoveryDeckProps): React.JSX.Element {
  ...
  function advance(): void {
    ...
    transitionTimer.current = window.setTimeout(() => {
      ...
      transitionTimer.current = window.setTimeout(() => setTransitionState("idle"), 0);
  ...
  return <><section className="public-discovery-deck" data-testid="public-discovery-deck" tabIndex={0} onKeyDown={onKeyDown} aria-label={`${label} 카드 탐색`}>
    ...
    {current === null
      ? <div className="deck-finish" role="status"><strong>살펴볼 카드를 모두 봤습니다.</strong><p>좋아요한 앨범은 아래에서 다시 열 수 있습니다.</p><button type="button" className="discovery-open" onClick={restart}>처음부터 보기</button></div>
      : <article className="discovery-card" data-transition-state={transitionState} aria-label={`${current.title}, ${current.artist}`}>
        <AlbumArt album={current} size="hero" priority />
        <div className="discovery-card-copy"><p className="entry-eyebrow">{current.primaryType === "EP" ? "EP" : "앨범"}</p><h3>{current.title}</h3><p>{current.artist}</p>{current.publicCurationReason !== undefined && <p className="public-reason">{current.publicCurationReason === "shared-tag"
          ? <>아카이브에 쌓인 <span className="keep-together">“{current.sharedMusicBrainzTag}”</span> 흐름과 이어집니다.</>
          : "아카이브가 다뤄 온 아티스트의 다른 앨범입니다."}</p>}
          <button className="discovery-open" type="button" onClick={() => onOpenAlbum(current)}>{current.title} 수록곡 보기</button></div>
      </article>}
    ...
    <p className="visually-hidden" aria-live="polite">{message}</p>
      ...
      <div>{likes.map((album) => <article key={album.releaseGroupMbid} className="liked-album-row"><AlbumArt album={album} /><div><strong>{album.title}</strong><span>{album.artist}</span></div><button type="button" onClick={() => onOpenAlbum(album)}>{album.title} 수록곡 보기</button></article>)}</div>
    </section>}</>;
}
