...
test("Given a public visitor, when the archive opens, then service trust pages and a restrained heading rhythm are available", async ({ page }) => {
  ...
  await expect(page.locator('.archive-navigation a[href="/method"]')).toHaveAttribute("aria-current", "page");
  ...
  await expect(page.locator('.archive-navigation a[href="/method"]')).toHaveCount(1);
  await expect(page.locator(".archive-navigation [aria-current=page]")).toHaveText("개인정보 처리");
  await expect(page.getByRole("navigation", { name: "이 페이지의 내용" })).toBeVisible();
  ...
  await expect(page.locator('.archive-navigation a[href="/method"]')).toHaveCount(1);
  await expect(page.locator(".archive-navigation [aria-current=page]")).toHaveText("이용 조건");
  await expect(page.getByRole("navigation", { name: "이 페이지의 내용" })).toBeVisible();
